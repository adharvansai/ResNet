...
class ResNet(nn.Module):
    def __init__(self,
        ...
        ### YOUR CODE HERE
        # define conv1
        self.start_layer = nn.Conv2d(3,first_num_filters,3,padding = 1,stride=1,bias = False)
        ### YOUR CODE HERE
        ...
        for i in range(3):
            ...
            self.stack_layers.append(stack_layer(filters, block_fn, strides, self.resnet_size, input_filters))
...
class standard_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.first_layer = nn.Conv2d(filters,filters,3,padding=1)
        self.second_layer = nn.Conv2d(filters,filters,3,padding=1)
        self.residual_projection = None

        if projection_shortcut is not None:
            self.residual_projection = nn.Conv2d(first_num_filters,filters,1,stride=strides)
            self.first_layer = nn.Conv2d(first_num_filters,filters,3,padding=1,stride = strides)
        
        #self.resblock = nn.Sequential(self.first_layer,nn.ReLU(),self.second_layer)
            ### YOUR CODE HERE
        self.bnrelu = batch_norm_relu_layer(filters)
        ### YOUR CODE HERE
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        out_l1 = self.first_layer(inputs)
        out_l1_bnrelu = self.bnrelu(out_l1)
        out_l2 = self.second_layer(out_l1_bnrelu)
        out_l2_bnrelu = self.bnrelu(out_l2)
        #pre_res = self.resblock(inputs)

        if(self.residual_projection is not None):
            res_input = self.residual_projection(inputs)
            print(res_input.size())
            res_add = out_l2_bnrelu + res_input
        else:
            res_add = out_l2_bnrelu + inputs
        out = res_add
        return out
...
class bottleneck_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.first_layer = nn.Conv2d(filters*4,filters,1,padding=0)
        self.second_layer = nn.Conv2d(filters,filters,3,padding=1)
        self.third_layer = nn.Conv2d(filters,filters*4,1,padding=0)
        self.residual_projection = None

        if projection_shortcut is not None:
            self.residual_projection = nn.Conv2d(first_num_filters,filters*4,1,stride = strides)
            self.first_layer = nn.Conv2d(first_num_filters,filters,1, stride = strides)
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        preact_l1 = self.bnrelu(inputs)
        out_l1 = self.first_layer(preact_l1)
        preact_l2 = self.bnrelu(out_l1)
        out_l2 = self.second_layer(preact_l2)
        preact_l3 = self.bnrelu(out_l2)
        out_l3 = self.third_layer(preact_l3)
        res_out = out_l3 + preact_l3
        out = F.relu(res_out)
...
class stack_layer(nn.Module):
    ...
    def __init__(self, filters, block_fn, strides, resnet_size, first_num_filters) -> None:
        ...
        if(block_fn.__name__ == 'standard_block'):
            self.resnet_size = resnet_size
            self.first_block = standard_block(filters,1,strides,first_num_filters)
            self.rest_blocks = standard_block(filters,None,strides,first_num_filters)
        elif(block_fn.__name__ == 'bottleneck_block'):
            self.resnet_size = resnet_size
            self.first_block = bottleneck_block(filters,1,strides,first_num_filters)
            self.rest_blocks = bottleneck_block(filters,None,strides,first_num_filters)

        ### END CODE HERE
    
    def forward(self, inputs: Tensor) -> Tensor:
        ### END CODE HERE
        blocks = self.first_block(inputs)
        for i in range(self.resnet_size-1):
            blocks = self.rest_blocks(blocks)
        return blocks
